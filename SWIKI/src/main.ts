...
${currentRawMarkdown}`;
  ...
  // Safety net for static-hosted wiki links: keep internal Markdown pages in hash routing.\n  // This catches links that may be emitted by plugins or navigation content before\n  // transformLinks() has a chance to normalize them.\n  document.addEventListener('click', (event) => {\n    const mouseEvent = event as MouseEvent;\n    if (mouseEvent.defaultPrevented || mouseEvent.button !== 0 || mouseEvent.metaKey || mouseEvent.ctrlKey || mouseEvent.shiftKey || mouseEvent.altKey) return;\n\n    const target = event.target as HTMLElement | null;\n    const link = target?.closest<HTMLAnchorElement>('a[href]');\n    if (!link) return;\n\n    const rawHref = link.getAttribute('href') || '';\n    if (!rawHref || rawHref.startsWith('#') || rawHref.startsWith('mailto:') || rawHref.startsWith('tel:')) return;\n\n    let url: URL;\n    try {\n      url = new URL(rawHref, window.location.href);\n    } catch {\n      return;\n    }\n\n    if (url.origin !== window.location.origin || !/\\.(md|markdown|mdown)$/i.test(url.pathname)) return;\n\n    // Convert /SWIKI/wiki/foo.md (or /wiki/foo.md) into the router's #/wiki/foo.md form.\n    const appRoot = new URL('./', window.location.href).pathname;\n    let routePath = url.pathname;\n    if (appRoot !== '/' && routePath.startsWith(appRoot)) {\n      routePath = routePath.substring(appRoot.length);\n    } else {\n      routePath = routePath.replace(/^\\/+/, '');\n    }\n\n    routePath = decodeURIComponent(routePath);\n    event.preventDefault();\n    router.navigate(routePath, url.hash ? decodeURIComponent(url.hash.substring(1)) : '');\n  });\n\n  // Setup Theme Palette Dropdown Chooser
